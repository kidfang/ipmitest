...
for (( h=$host_f; h<=$host_n; h=h+1 ));
do

nvme_num=$( nvme discover -t rdma -a "$s260_ip" -s 4420 -q host"$h".nqn | grep -i subnqn | wc -l )

for (( i=1; i<=$nvme_num; i=i+1 ));
        do
		sub_nqn=$( nvme discover -t rdma -a $s260_ip -s 4420 -q host"$h".nqn | grep -i subnqn | sed -n "$i"p | awk '{print $2}' )
		nvme connect -t rdma -n $sub_nqn -a $s260_ip -s 4420 -i 8 -q host"$h".nqn
	done
