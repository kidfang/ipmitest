...
#		typeset -u Record_length
		Record_type=$(echo $SDR_con | cut -f 6 -d " ")
#		Record_length=$(echo $SDR_con | cut -f 7 -d " ")
...
#		HEX_to_DEX=$(echo "obase=10;ibase=16;$Record_length"|bc)
		Sensor_name_HEX=$(echo $SDR_con | cut -f $Start-100 -d " ")
		Sensor_name_ASCII=$(echo $Sensor_name_HEX | xxd -r -ps)

		echo "$i    $DEX_to_HEX    $next     $Sensor_name_ASCII   Record_type: $Record_type"
	done
